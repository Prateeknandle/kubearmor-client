...
func Collect(c *k8s.Client, o Options) error {
	...
	// KubeArmor DaemonSet
	errs.Go(func() error {
		v, err := c.K8sClientset.AppsV1().DaemonSets("kubearmor").Get(context.Background(), "kubearmor", metav1.GetOptions{})
		if err != nil {
	...
	// KubeArmor Pod
	errs.Go(func() error {
		pods, err := c.K8sClientset.CoreV1().Pods("kubearmor").List(context.Background(), metav1.ListOptions{
		...
		for _, p := range pods.Items {
			...
			v := c.K8sClientset.CoreV1().Pods("kubearmor").GetLogs(p.Name, &corev1.PodLogOptions{})
